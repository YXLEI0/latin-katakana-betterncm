...
| 顺序 | 来源 | 例子 | 可信度 / 行为 |
| --- | --- | --- | --- |
| 1 | **离线词典**（`src/core/dict.js`，**6057 条**） | `clover` → クローバー、`hello` → ハロー | 确定对，直接用 |
| 2 | **罗马音切分**（歌词里官方写的罗马音） | `sekai` → セカイ、`shinjiteru` → シンジテル | 确定对，直接用 |
| 3 | **大模型校正**（配了 API Key 时；**带整句歌词当语境**） | `kaleidoscope` → カレイドスコープ | 很准；**等待期间先用规则读音当暂定值**（注音淡一点），结果回来就地改写 |
| 4 | **免费接口**（没配大模型时才轮到它） | Google en→ja，**只接受纯片假名** | 同上 |
| 5 | **英文音译规则**（垫底） | `blorf` → ブローフ | 猜的；只有 3/4 给不出、失败或没配时才用 |

**为什么在线那两层排在规则前面**：规则是拼写音译（`hello` → ヘッラオ、
`question` → クワエサション），那不是人唱的音。所以词典外的词优先等在线结果。

**但等待期间不空着**：先用规则读音当**暂定值**（注音加 `lt-pending` 类、淡到 45%），
结果回来由 `annotate.relabel()` **就地改写**（DOM 节点一个都不动）。
早期版本是"先不标"，结果一行里只要有一个词在等就整行空着，而且首个词所在的
文本节点已有记录、后面拿到结果也补不上 —— 用户看到的就是"全英文行标注后有概率消失"。
