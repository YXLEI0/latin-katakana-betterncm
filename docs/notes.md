...
| 3 | **大模型校正**（配了 API Key 时；**带整句歌词当语境**） | `kaleidoscope` → カレイドスコープ | 很准；**等待期间先不标**，回来再补 |
| 4 | **免费接口**（没配大模型时才轮到它） | Google en→ja，**只接受纯片假名** | 同上 |
| 5 | **英文音译规则**（垫底） | `blorf` → ブローフ | 猜的；只有 3/4 给不出、失败或没配时才用 |

**为什么在线那两层排在规则前面**：规则是拼写音译（`hello` → ヘッラオ、
`question` → クワエサション），那不是人唱的音。所以词典外的词**先不标**，
等准确的读音回来再补 —— 晚半秒标一个对的，好过先标一个错的再改。

**底线**：在线那层没配 key / 正在退避 / 请求失败时，`isWaiting()` 立刻变 false，
规则马上兜底 —— 断网不会变成一个词都不标（免费那层请求失败时也会通知上层重扫，
否则"先不标"的那一轮永远补不回来）。
