...
(function (root, factory) {
  ...
  /*
   * 词尾 -ize / -yze：英语里一律读 /aɪz/（memorize・organize・analyze・realize），
   * 所以整块读「辅音 + アイズ」，前面的辅音要拉到 a 上（memorize = メモ + ライズ）。
   *
   * 单列一条是因为规则层原本把 -ize 当普通拼读，读成「ゼ」，
   * 一整类词（organize オーガニゼ、apologize アポロギゼ、analyze アナライゼ…）
   * 全错。用户截图里 `memorize` 被标成 メモリゼ，那是罗马音层抢先给的。
   * 软音 g/c 要按 ジャ/サ 走（apologize アポロジャイズ、criticize クリティサイズ）。
   */
  var EN_IZE = /^([a-z]*?)([bcdfghjklmnpqrstvwxyz])(iz|yz)(e[sd]?|ing)$/;

  /** 同一个形状，用来在罗马音层拒绝它（词干可以是空的：size / prize） */
   ...
   *
   * 单列一条是因为罗马音层会把 `jude` 切成 ju-de 读成 ジュデ（用户截图里的
   ...
   * 只有三条特判，都在 tryRomajiToken 里：促音（kk/tt/…）、长音对（ou/oo/…）、
   * 拨音 n（后面不是元音或 y 时收 ン）。
  ...
  // 该页的总纲是日语借词优先采用英式发音（vitamin -> ビタミン 而不是
   ...
   * 常见外来语小表（英文 -> 片假名）。
   *
   * 和 ENGLISH_EXCEPTIONS 分开放，因为性质不同：前者是拼写本来就不规则
   * （light / through / love），这张表是拼写规则、但规则会读歪或读土的高频外来语。
  ...
  // 命中条件写在 2 号规则里：r 后面是词尾或辅音；r 后面是元音时
  ...
  // th 本身也算：θ -> サ行、ð -> ザ行 都有明确落点（页面里都写了），
  // 但哪个词是 θ、哪个是 ð，拼写分不出来。
  ...
  // 首页例子 cat -> キャット、dog -> ドッグ。该页同时说明多音节词里促音只落在
  // 重读音节上，而重音光看拼写定不下来 —— 所以只对单音节词动手
  ...
  // vitamin -> ビタミン、video -> ビデオ）。ヴァ/ヴィ/ヴェ/ヴォ 是备选写法，
  // 只在日语实际那么写的词里用，那些词由例外表和小表兜住 —— visual 就是。
  var EN_CONSONANT = {
    b: "\u30D0",
    c: "\u30AF",
    d: "\u30C0",
   ...
   * 所以这里老实把每个辅音的五段写出来。
   * 不能直接借 EN_PAIR：那里带上下文特例（wo = ウォ、bi = ビ、
  ...
  /** 这两个字母能不能拼成一截「辅音 + l/r」连缀（clover 的 cl） */
  ...
  function convertEnglish(word) {
    ...
    while (i < len) {
       ...
       * 这几组在英语里从不发音（knock/knee/knight、wrap/write/wrong、
       ...
       *          只处理词尾这一种：number / amber / timber 的 b 是发音的
       *          （ナンバー・アンバー・ティンバー），而 plumber 又哑 —— 词中的 mb
       *          分不出来，那些交给词典。
       */
      ...
      if (rest >= 2) {
        pair = word.substr(i, 2);
        // ca 在闭音节里读 キャ（参照 sljfaq：æ after k -> キャ，cap -> キャップ）；
  ...
  /*
   * 字母名（日语里的通行读法）。用于 `D/N/A`、`N/A`、`A.B.C`、`R&B` 这类记号：
  ...
  function mergeContraction(kana, suffix) {
    ...
    if (suffix === "s") {
       ...
       * 注意去掉的是词尾那个假名再补促音那套 —— 词干本身可能已经带 ッ
   ...
   *
   * 为什么判据必须是"每个分段都是单个字母"：`X-Y` 是记号（エックスワイ），
  ...
  function spellOutAcronym(raw, dict, enWords) {
    ...
    /*
     * 有元音的这一支只收 2~3 个字母：`SOS` / `QTE` / `YY` 是缩写，
     * 而 4 个字母以上、又带元音的（`DIVA` / `QUIX` / `KISS`）基本都是词或名字，
  ...
  var FIRST_ROWS = {
     ...
     * 那一支在 looksLikeTransliteration 里按词首是不是 th 单独放行 ——
  ...
  function looksLikeTransliteration(word, kana) {
     ...
     * 只对 th 开头的词放行ダ行 —— 不然 `tick` -> ダニ（蜱虫，意译）这种
  ...
  function createReader(opts) {
    ...
    var LAYER_FN = {
      dict: function (c) {
        ...
        // 注意不能再走"去掉非字母"那一档键：`déjà` 会被削成 `dj`，
      ...
      romaji: function (c) {
        /*
         * 词尾 -ize / -yze 的不是日语罗马字（日语里没有以 ぜ 收尾的动词），
         ...
         * 这种英文词会被它当成日语罗马字。而层序里罗马音排在大模型前面，
         * 它一答，大模型就没机会纠正 —— 这些词就永远读错（用户报的
    ...
    function lookup(raw) {
       ...
       *
       * 必须排在词典前面：记号的字母串去掉分隔符之后可能正好是个词条
       * （dna / abc 就在词典里），那会读成单词音而不是字母名。
       * 判据用原始串（raw）而不是折过的 shown —— 折的时候连字符会被去掉，
      ...
      /*
       * ①.2 同一个元音重复成串（`AAAAA` / `OOO` / `aaa`）：那是喊叫/拖长音，
       *      不是词，直接按那个元音叠出来。用户截图：
      ...
      if (vowelRun && vowelRun[0].length <= 12) {
        ...
        if (oneKana) {
          ...
          /*
           * source 用 letters：它和记号/字母名一样属于形态层的确定答案 ——
      ...
      /*
       * ①.6 只有长音符的词（`Tōkyō` / `kōhī` / `arigatō`）：这就是日语罗马字，
