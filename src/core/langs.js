 ...
 *
 * 分工（一个关注点只放一个地方）：
 *   core/letters.js  切词：哪些字符算西文的一个词（拉丁 / 西里尔 / 希腊三种字母）
 ...
 *
 * 对外的四个函数：detect(text) 判这一行是什么语言（判不出来返回 null）；
 * fits(id, text) 是整首投票的兜底 —— 短行自己分不够时看它像不像那种语言；
 * toKatakana(id, word) 按那种语言把词读成片假名；word(id, raw) 查这个词有没有
 * "日语里就是这么写"的借词读音。
 *
 * 借词表那份「日语里就是这么写的」（core/loan.js）是日语通行写法，相当于人工词条，
 * 所以在外语行上比英语词典还优先。
 *
 * 两种字母：拉丁字母的词由 core/letters.js 切出来，读音走这里的引擎；
 * 西里尔和希腊字母 core/letters.js 一样会切（它认这三种字母），但词典层和罗马音层
 * 都读不了 —— 俄语和希腊语的支持就落在"这里必须给出答案"上。
 *
 * 规则层的结果一律 confident:false（拼写近似，配了 key 交给大模型按整句定，判完还会
 * 被沉淀成离线词条），借词表命中的是 confident:true。
 *
...
(function (root, factory) {
   ...
   * 时它可能不在，取不到就退化成"没有这张表"。
   * 本文件只依赖 core/loan.js，拼读判据全在自己这里，不伸手去拿 core/reading.js。
   */
  function dep(name) {
   ...
   * ASCII 连字符也要去掉（rendez-vous→rendezvous、Auf-Wiedersehen→aufwiedersehen），
   * 借词表的键都是折掉的写法，法语那张表里 rendez-vous / l'eau 这类词全靠这条才查得到。
   */
  function bare(s) {
    return lower(s)
      .replace(/['\u2019\u2011\u2013\u2014\u30FB.\uFF0E-]/g, "")
   ...
   * 返回 { col, kana, len, long, glide }，col 是拼到辅音哪一列，glide 是滑音假名。
   */
  ...
  function germanToKatakana(raw) {
    ...
    while (i < len) {
      ...
      // ng / nk 只在不是元音前才是 ング / ンク（lange ランゲ、singen ズィンゲン）
  ...
  // ============================================================ 拉丁语
  //
  // 用的是古典式读法（日语里的拉丁语惯例：Caesar カエサル、Cicero キケロ），
  // 不是教会式，用户截图里的参考答案就是古典式：
  ...
  function portugueseToKatakana(raw) {
    ...
    while (i < len) {
      ...
      // 鼻元音（包括 ão / õe 这两个典型词尾）：辅音 + ão 合成 サン / タン，如 coração コラサン
      ...
      if (c === "\u00E3" || c === "\u00F5") {
        var nasal = c === "\u00E3" ? "\u30A2\u30F3" : "\u30AA\u30F3";
        // 单独的 ã / õ 读 アン / オン（maçã マサン）；ãe / õe 上面已经接走了
        out += nasal;
        i += 1;
        continue;
      }
      ...
      if (two === "nh" || two === "lh") {
        ...
        if ("aeiou".indexOf(nlv) >= 0) {
          // 颚化那一拍用イ段假名做底 + 小元音（nh+a ニャ、lh+o リョ）
          var pal = pick(nrow, "i");
      ...
      // 辅音 + ão / õe：常常要合成一拍（coração コラサン、põe ポン）
   ...
   * 和德语一样要能拼进辅音那一拍：ij アイ（tijd タイト）、oe ウー（goed フート）、
  ...
  // ============================================================ 斯瓦希里语
  //
  // 斯瓦希里语是开音节语言（音节几乎全是 辅音+元音），这一点和日语一样，
  ...
  function swahiliToKatakana(raw) {
    ...
    while (i < len) {
      ...
      // 撇号（ng'ombe 这种）不发音
      if (c === "'" || c === "\u2019") { i += 1; continue; }
      // 鼻音 + 辅音 -> ン（mbingu ンビング、nchi ンチ）；ng / ny 是二合字母，交给下面。
      // w / y / h 前面不算这个词里的鼻音：mwa 读 ムワ、mya 读 ミャ
      var nx = w.charAt(i + 1);
   ...
   *   col  拼在声母后面时声母换到哪一列（shang -> シャ + ン，col = "a"）
   *   kana 声母那一拍之后再补的假名，`ang` 补 ン，`ai` 不补（シャ + イ 里的 イ
   *        由声母那一列自己带出来）
   * 复韵母里的 i/u/ü 是介音，日语音译并进声母那一拍（xie シェ、liu リュウ）。
   */
  var PY_FINAL = {
    a: { solo: "\u30A2", col: "a", kana: "" },
    o: { solo: "\u30AA", col: "o", kana: "" },
  ...
  function pinyinToKatakana(raw) {
    ...
    var col = fin.col;
    // j/q/x/y 后面的 ü 系列韵母写 u 列（ju ジュ、xu シュ）
    var body = row[col] || row.a;
    var kana = body + (fin.kana || "");
    // zhi / chi / shi / ri / zi / ci / si 里的 i 不读 イ（知 ジ、吃 チ、思 ス）：
  ...
  /*
   * 辅音 + 硬元音（а э о у）各拼一拍。日语假名本身自带元音，所以「辅音 + 元音」
   * 必须合成一个假名（то ト、не ネ），不能一个字母一个假名，
  ...
  /**
   * 词尾/连缀里读拨音（或不带元音那一拍）的辅音：
   *   н -> ン（拨音）
   *   д -> ド，不是 ドゥ：Александров アレクサンドロフ。ду 那种"辅音 + у"
   *        仍走 RU_ROW 的 ドゥ，不受影响
   */
  var RU_CLUSTER = { "\u043D": "\u30F3", "\u0434": "\u30C9" };
  /** 软元音 */
  var RU_SOFT_V = { "\u0438": "\u30A4", "\u0435": "\u30A7", "\u044E": "\u30E5", "\u044F": "\u30E3", "\u0451": "\u30E7" };

  /**
   * 词尾 -ый 的写法按辅音分：н 收 ヌイ（Красный クラースヌイ），
   * в 是 ヴィ（новый ノーヴィ），其余辅音按 RU_ROW 的 u 列 + イ。
   */
  var RU_Y_TAIL = { "\u043D": "\u30CC\u30A4", "\u0432": "\u30F4\u30A3" };
  ...
  function russianToKatakana(raw) {
    ...
    while (i < len) {
      ...
      var prev = i > 0 ? w.charAt(i - 1) : "";
      // 再往后一个（判断词尾 -ый / -ий 时要用）
      var after = w.charAt(i + 2);

      /*
       * 词尾的 -ий / -ый：й 不单独成拍。
       *   -ий -> 软音那一拍 + 长音（Русский ルースキー、синий スィーニー）
       *   -ый -> н 是 ヌイ（Государственный ゴスダールストヴェンヌイ、
       *          красный クラースヌイ），в 是 ヴィ（новый ノーヴィ），其余 u 列 + イ
       * 这两条必须排在"辅音 + ы / 软元音 и"前面 —— 否则 ы/и 先被吃掉，剩下的 й
       * 又读一拍，尾巴就多出一个 イ（用户截图：Государственный 原来读成 …ヴェヌイイ）。
       */
      if (i + 3 === len && after === "\u0439" && RU_ROW[c]) {
        if (next === "\u0438" && RU_I[c]) { out += RU_I[c] + "\u30FC"; i += 3; continue; }
        if (next === "\u044B") { out += RU_Y_TAIL[c] || RU_ROW[c].u + "\u30A4"; i += 3; continue; }
      }
      // 词尾的 -ий / -ый 读 イー（ロシースキー）—— 辅音后面那种走上面那条
      ...
      // ---- 辅音
      /*
       * 词尾的 в 清音化读 フ：日语里俄语姓氏的 -ов / -ев 就是这么写的
       * （Александров アレクサンドロフ、Иванов イワノフ、Киев キエフ）。
       * 用户截图：Александров 原来读成 …ロヴ。
       * 单字母的那个前置词 `в`（"在…里"）不算词尾，照旧 ヴ（用户用例 6 的 `в веках`）。
       */
      if (c === "\u0432" && i + 1 === len && len > 1) { out += "\u30D5"; i += 1; continue; }
      if (next !== "" && RU_SOFT_V[next] !== undefined && RU_I[c]) {
        /*
         * е 只在 н / т 后面软化（нет ニェト、теле チェレ）；
         * д / с / з / б / п / м / в / ф / л 后面保持硬音
         * （Денис デニス、себя セビャ、Александров アレクサンドロフ ——
         *  л + е 在日语里写作 レ，不写 リェ）。
         */
        var softEnable = next !== "\u0435" || c === "\u043D" || c === "\u0442";
        if (softEnable) {
          out += RU_I[c];
      ...
      /*
       * 双写辅音：н 收一拍拨音，其余只读一次。
       *   нн -> ン + ナ行（Государственный ゴスダールストヴェンヌイ、Анна アンナ）
       *   其余 -> 一拍（программа プログラム）
       * 用户截图：Государственный 原来读成 …ヴェヌイイ（既丢了拨音又多了尾巴）。
       */
      if (c === next && RU_ROW[c]) {
        if (c === "\u043D") out += "\u30F3";
        i += 1;
  ...
  /**
   * 希腊语的辅音行：辅音 + 元音要合成一拍（λα ラ、νο ノ、ρω ロ）。
  ...
  var SIGNALS = {
     ...
     * 唯一特别的是门槛：法语和拉丁语的词形太像（Non, le grand amour ne suffit
   ...
   * 但它们正是判据），表里没有而且是英语常用词的才倒扣 —— 而且不给形状分
  ...
  function wordScore(id, w) {
     ...
     * 而且不给形状分（`you` 也满足"元音收尾"，给了形状分就把英语惩罚抵消了）。
   ...
   * 每个语种后面还有自己的门槛（拉丁语要强信号、斯瓦希里语要真的命中一个
  ...
  function detect(text) {
    ...
    if (!s) return null;
    /*
     * 假名优先：这一判必须排在最前面（西里尔 / 希腊那两个"看字母表就认定"
     * 的判断原来在它前面）。用户截图 `無限増幅回路（Ω）` 暴露的就是顺序问题 ——
     * 日语行里夹一个 `Ω`，整行被判成希腊语，单字母 Ω 走希腊引擎读成 オ，
     * 而用户要的是单位读法 オーム。假名优先之后这类行照旧是日语行。
     */
     ...
     * 那一行是日语，不是拉丁语。这一判在函数开头（顺序很要紧，见那里）。
     */
  ...
  function passes(id, text, score) {
    ...
    var s = lower(text);
    /*
     * 拉丁语要强信号：弱信号（-is/-at 结尾、et/in 这类虚词、qu 组合）
  ...
  /**
   * 这一行"像不像"某种语言（给整首投票兜底用：短句 `Ukuu ukuu` / `Dominatus`
   * 自己分数不够，但整首都是那种语言时应该按那种语言读）。
   * 判据：词形命中率够高，而且没有英语常用词（有就一票否决）。
   */
