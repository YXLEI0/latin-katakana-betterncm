 ...
 * （规则给 ヘッラオ / クワエサション，没人这么唱）。实测大模型给的读音质量
 * 碾压规则，所以在规则层之后再加这一层：规则先给一个即时结果，
 * 大模型的结果异步回来了再把它换掉。
 *
 * 与 core/correct.js（Google 翻译接口）的分工：这一层要用户自己填 API key，
 * 但质量明显更好、能处理虚词（the -> ザ）与变形词；没填 key 就整层不工作，
 * 自动退回 correct.js 那条免费路子，功能不残缺。两层都只收纯片假名的返回值
 * —— 唱歌要的是读音，不是翻译。
 *
 * 设计要点：
 *   1. lookup() 永远同步返回（可能是 null），不阻塞 DOM 注音；
 *   2. 没命中的词进队列，攒满 FLUSH_DELAY_MS 或 BATCH_SIZE 个再一次性问，
 *      同一个词只问一次（命中与"模型也没给"都记进缓存，永久保存）；
...
(function (root, factory) {
   ...
   * 就是被它掐掉的（用户截图里的报错）。一批几十个词、每条都带整句语境，
   * 服务商忙的时候首字节就得等十几秒，20 秒太紧；45 秒稳一些，
   * 而且超时之后我们还会把批次减半再试（见 send/fail 里的 batchCap）。
   */
  var REQUEST_TIMEOUT_MS = 45000;
  var MAX_REQ_PER_MIN = 20; // 限流：再准也不该把页面拖垮
  var COOLDOWN_MS = 60000; // 失败后的退避起点
  /*
   * 退避上限。原来写的是 10 分钟，真机上太伤了：接口抖一下或者撞一轮限流，
   * 就会进入"读音全都没矫正"的状态（本地读音还在，但一条都不再问模型），
   * 用户以为插件坏了，而且一等等十分钟。3 分钟够礼貌，也能自己恢复。
   */
  var COOLDOWN_MAX_MS = 3 * 60 * 1000;
  var MAX_WORD_LEN = 24;
  /** 上下文（整句歌词）最长留多少字符 —— 免得把整个歌词本塞进提示词 */
  ...
  /**
   * 把用户填的接口地址折成真正能 POST 的地址。
   *
   * DeepSeek / OpenAI 的文档里给的是 `base_url`
   ...
   * 提示词：带上下文。
   *
   * 孤立地问一个词，有些词根本定不下来 —— read 是 リード 还是 レッド、
   * live 是 ライブ 还是 リブ、「Love」是歌里的词还是人名。把整句歌词一起给它，
   * 判断依据就完全不同了（用户提的要求）。条目按下标编号（`items` 是
   * [{i, w, line}]），响应也用下标当键，同一个词出现在两句里时不会互相覆盖。
   */
  function promptFor(items) {
    ...
    return (
      "你是日语歌词注音助手。下面是日语歌里出现的西文词（拉丁字母 / 西里尔字母 / 希腊字母），" +
      "每一项带它所在的**整句歌词**" +
      ...
      "beat → ビート（不是「鼓動」）；\n" +
      "7. 词可能是**英语以外的语言**，一律按那门语言的读音音译，别按英语拼读：" +
      "法语（le ル、je ジュ、monde モンド、amour アムール）、德语（die ディー、ich イッヒ、" +
      "schön シェーン、Zeit ツァイト）、拉丁语（**古典式**：Vindicia ヴィンディキア、" +
      "Vanitatum ヴァニタトゥム、sentio センティオ、dolor ドロル）、葡萄牙语（ão アン、nh ニャ）、" +
      "荷兰语（ij アイ、oe ウー、huis ハウス）、斯瓦希里语（开音节直读：Shambulia シャンブリア）、" +
      "汉语拼音（zh ジ、x シ、q チ、liú リュウ）、俄语（Отчизну オチズヌ、Виват ヴィヴァト、" +
      "Анастасия アナスタシヤ）、希腊语（Θάλασσα サラッサ、ουρανός ウラノス）；\n" +
  ...
  /** 上下文规范化：折叠空白、截掉过长部分（只用来判"是不是同一句"和喂给模型） */
   ...
   *
   * 用在回答被截断（JSON 不完整）的时候：能对上的先收下，剩下的不算
   ...
   *   1. 粘的时候把引号一起带进来了：`"sk-xxx"`；
   *   2. 前后有空格/换行（从网页上复制很容易带上）；
  ...
  /**
   * 把"HTTP 4xx/5xx"变成能照着修的错误信息：地址 + 服务端原话。
   *
   * 404 最常见的原因是地址少了 `/chat/completions`（把 base_url 粘进来了），
   * 而服务端对这种情况往往回一个空 body —— 只报 "HTTP 404" 的话，
   * 用户完全不知道该改哪儿。
   */
  ...
  /**
   * 连 HTTP 状态码都没有的失败（fetch 直接 reject）：几乎都是网络或跨域问题，
   * 而 "Failed to fetch" 这句话本身对用户毫无信息量，得翻译一下。
   */
  ...
  function createClient(options) {
     ...
     * 光看"纯片假名"拦不住意译和拟声词 —— 用户报的 tick -> カチカチ 就是这种，
    ...
    /*
     * 词 -> 最近一次拿到的读音（不区分语境）。只给控制台排障用
     * （`WK.display('词')` 不带句子时总得有东西可看），不参与页面注音的判定 ——
     * 页面必须严格按当前这句的语境取读音。
     */
    var byWord = {};
     ...
     *
     * 答案校验刚刚放宽了：首音校验那套判据是按英语拼写定的，外语行上会误伤
     * （拉丁语 vacuum 的 ワクーム 就被判掉过，德语 w→ヴ、拉丁语 v→ヴ/ワ 这些
     * 对应关系英语里根本没有）。老 miss 里混着正确答案，而 miss 是永久的、
     * 还落了盘 —— 不主动清一次，那个词会永远停在规则层，用户截图里的
     * 「Vacuum 一直是黄的」就是这么来的。只清一次（用一个标记记住），
     * 命中的缓存一条都不动。
     */
     ...
     *
     * 队列本身不够：注音层只在注音那一刻问一次（annotate -> lookup），
     * 之后那个词就再没人问了。如果那一刻这一层正好不可用 —— 没配 key、退避中、
     * 刚启动配置还没读进来 —— 那个词就永远停在规则层，用户看到的是
     * 「这个单词一直是黄的」（`sieh` 就这么一直黄着）。有这张表，就能在任何时刻
     * 把它们捡回来补问（见 sweep）。命中或"模型明确给不出"（miss）都算结论，从表里删掉。
     */
    var wanted = new Map();
    ...
    var cooldownUntil = 0;
    /*
     * 自适应批次上限：0 = 用 cfg.batchSize。请求超时（用户截图里的
     * "The user aborted a request."）几乎都是"这批太大 / 服务商在排队"，
     * 超时之后把上限减半再试，成功率立刻高很多，成功之后恢复原大小。
     * 只减不加，不会来回震荡。
     */
    var batchCap = 0;
    ...
    function saveCache(force) {
      ...
      // 显式保存（force）要同步落盘：调用方（比如关闭页面、或者测试）期望
    ...
    function keyOf(word) {
       ...
       * 优先 NFD 分解再删组合用记号；引擎不支持时退回"把带符号的字母整段删掉"
       * （宁可少问一个词，也不拿半个词去问）。
       */
      ...
      /*
       * 只留西文字母：拉丁（含 NFD 折过之后剩下的）、西里尔、希腊。
       *
       * `[^a-z]` 那一版会把西里尔和希腊的整词削成空串 —— keyOf 返回 ""、
       * cacheKeyOf 返回 ""，lookup 直接 return null，俄语和希腊语的词从来没被问过，
     ...
     * 缓存/队列的键：词 + 它所在的那句话。
     *
     * 带上语境是因为只按词缓存的话，先遇到哪句就把哪个读音钉死一辈子
     * （`read` 在 “read a book” 里是 リード、在 “I read it” 里是 レッド）。
    ...
    function schedule(delay) {
      ...
      /*
       * 在退避里不能直接 return（老版本就是 `if (!canAsk()) return;`）：
       * 退避期间 canAsk() 为 false，那次 schedule 就什么都不做。而退避结束时
       * 没有任何事件会再来叫我们（页面不动就没有新的 lookup），队列就这么一直躺着 ——
       * 用户看到的是「读音全都没矫正」，面板上写着「队列里还有 184 个词在等，
       * 本分钟还剩 20 次额度」，额度是满的却没有请求在飞，正是用户截图里的样子。
       * 所以要排到退避结束的那一刻去重试。
       */
      if (!canAsk()) {
    ...
    function send() {
      ...
      return Promise.resolve(p)
        ...
        .then(function (data) {
           ...
           * 退避重发同一批，于是再失败 —— 那些词就永远停在规则层（用户看到的
    ...
    function applyBatch(batch, obj, partial) {
       ...
       */
      var byIndex = {};
      // 注意名字：不能叫 byWord —— 外层那个 byWord 是"词 -> 最近读音"的索引，
      ...
      for (var i = 0; i < batch.length; i++) {
        ...
        if (!why) {
          ...
          /*
           * 「回答被截断」时抢出来的那几条：没抢到的不记 miss —— miss 是永久的
           * （还落盘），只有"模型明确给不出/给错了"才该记。截断是我们这边的批次太大
           * 造成的，记成 miss 就等于因为一次截断把那些词永久钉在规则层。
           * 它们没进 mem，这里直接放回队列重问（见下面 partial 那一支）。
           */
          if (partial) {
            /*
             * 直接把没抢到的条目放回队列，而不是指望"下一轮 lookup 再来问"。
             *
             * 为什么必须现在放：注音层只在注音那一刻 lookup 一次，那一轮之后
           ...
           *
           * 注意它是永久的（会落 localStorage）—— 模型当时给的答案被我们判掉了，
    ...
    function fail(batch, err, usage) {
       ...
       * 是暂定（`wk-pending`，淡到 45%），失败后进入退避、isWaiting 变成 false，
    ...
    function lookup(word, line) {
      ...
      if (!canAsk()) {
        /*
         * 这一层暂时不可用。退避到期要排一次补问 —— 不然那些词要等页面重扫
     ...
     * 退避结束没有任何事件会来叫我们 —— 那些词就一直是黄的。这里排一个
     ...
     * 这个词现在是不是"还在等结果"？
     *
     * 上层靠它决定要不要先拿英文音译规则的结果顶上：
     * 用户要的顺序是「大模型 -> 免费接口 -> 规则」，所以等待期间先不标，
    ...
    return {
       ...
       *
       * 缓存是按「词 + 语境」存的，这里按词归并：同一个词在几个不同句子里
       ...
       *
       * 什么时候必须点它：模型当时的答案被我们判掉了（首音校验误伤、模型抽风、
      ...
      retryMisses: function () {
        ...
        /*
         * miss 清掉之后要重新排队：这一类词是"问过但没收下"，mem 里刚被删掉，
