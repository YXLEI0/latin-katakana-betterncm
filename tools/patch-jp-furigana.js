 ...
 *
 * 判断这一行有没有被外人动过。我们在同一行上插 <ruby>，它就判定行脏 → 还原 →
 * 重建整行 → 我们的注音被抹掉 → 我们重注 → 无限来回（实测轨迹里 changed=18
 * restored=18 每秒四次，肉眼就是抽搐）。
 *
 * 只需要放宽这一条：我们改写的是它留着的原文本节点（只切短、不删除），所以它
 * 算出来的"看得见的原文" hostsText() 一个字符都没变 —— 用它的真实 DOM 结构验证过，
 * 注解前后它读到的都是 "取戻したい　ヒーローみたいに"。只要它别因为"多了一个子节点"
 * 就把整行推倒重来，两种注音就能安稳共处。
 *
 * 补丁做两件事：isClean() 的子节点计数忽略我们插的节点（带 kt-ruby / kt-rt 的）；
 * restore() 拆 wrap 时先把我们挂在它 wrap 里的注音节点搬到 host 上，免得跟 wrap
 * 一起被丢掉（它随后 buildWrap 会用 include 把 host 的全部子节点搬进新 wrap，
 * 我们的节点也跟着进新 wrap）。
 *
 * 用法：
 ...
 *
 * 版本：v1 只认 kt- / lt-（本插件改名前的 DOM 前缀）；v2 改认 kt- / lt- / wk-
 * （本插件现在的前缀），并且认 __wkOwned 标记位。装上的是 v1 时，直接重跑一次
 * 本脚本就会就地升级（只改类名枚举和标记位）。
 *
 * jp-furigana 更新后补丁会丢失，重新执行一次即可。
 */
"use strict";
...
/*
 * v1 的标记：装上的是 v1（外来节点判据只认 kt- / lt-）时，applyPatch 会就地升级成 v2，
 * 而不是报错让人先去 --revert —— 升级要改的只有两处类名枚举和一个标记字符串。
 *
 * 为什么要升级：本插件的 DOM 前缀从 lt- 改成了 wk-（latin-katakana →
 * western-katakana），老 v1 补丁认不出 wk-ruby，我们的注音会让那行被判脏并重建 →
 * 一直闪。v2 的判据同时认 kt-（片假名终结者）/ lt-（本插件改名前的节点）/ wk-（现在）。
 */
...
const PATCHES = [
  ...
  },
  {
     ...
     * 这才是"闪烁"的真正来源：不是判定逻辑，而是变更通知把行标脏了。
     */
  ...
  },
  {
     ...
     * 两边永远对不上 → isClean() 永远 false → 它每一轮 pass 都重建这一行 →
     * 我们的注音每轮被抹掉 → 一直闪。
     *
  ...
  },
  {
     ...
     *
     * 同步补就没有这个空窗：我们被它直接调用，在同一个任务里把注音插回新 wrap，
 ...
 *
 * 还原要逐字节回到原始，不能只把两处替换退回去 —— 我注入的 helper 块
...
function main() {
  ...
  // 备份只在没有备份时写：升级（v1 → v2）时不能用当前这份 v1 覆盖掉原始备份，
