 ...
 * 切法：把歌名切成"西文词"和"其它"（假名 / 汉字 / 数字 / 符号 —— 这些当**通配段**，
 * 吃多少假名都行，代价 0），再在官方读音上找一条最省的切分（DP + 假名编辑距离）。
 * 切开之后只在**这一首歌**里生效 —— 万一切错了，影响面就是那一首，不会污染别的歌
 * （这也是"整首专属"这一档存在的意义）。
 *
 * 歌名可以是纯西文（`Beat Eater`）、西文 + 假名汉字（`potatoになっていく`、
 * `サンドリヨン 10th Anniversary`）、带符号（`Vampire's ∞ pathoS`）——
 * 后两种靠通配段对齐，切出来还要过可信度检查，过不了就整条不要。
 *
...
/**
 * 把整串官方读音按段切开：西文段尽量像它的候选读音，通配段吃什么都行（代价 0，
 ...
 *   - **假名段**：读音就是它自己（平假名折成片假名）—— 比如 `potatoになっていく`
 *     里的 `になっていく`。它把"通配段"的自由度收掉大半，切分才可信；
 *   - **通配段**：汉字 / 数字 / 别的符号，吃多少假名都行（代价 0）。
 * 纯空白和纯标点的空隙**不产生段**（歌名里的空格 / 连字符不发音，
 * 有了段反而会让 DP 把词的边界挪走，`from Y to Y` 的 Y 就会被切错）。
 * `&` / `＆` 是唯一有读音的符号（アンド）。
 */
...
function generate() {
  ...
  for (const row of rows) {
     ...
     *   ② 长度和我们候选差不超过 1 拍（有通配段时允许它多吃，但不许把词挤没）；
     *   ③ 与我们候选的编辑距离不超过 1（通配段让"硬凑"变得太容易，这里要更严）。
     */
    let bad = null;
    segs.forEach((s, i) => {
