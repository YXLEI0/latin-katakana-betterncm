...
- 读音按顺序取：**离线词典**（6077 条）→ **罗马音** → **大模型校正** → **免费接口** → **英文音译规则**，这个顺序**可以自己调**（设置面板「读音来源顺序」的 ↑↓）：把*英文音译规则*提到在线层前面 = 一个请求都不发（纯离线）；把*大模型*提到词典前面 = 连词典命中的词也让模型判一遍。规则只当垫底：在线那层还没回来时先给一个**暂定的规则读音**（样式淡到 45%，`lt-pending`），拿到结果**就地改写**、不重建节点
