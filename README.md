...
- 读音按五层顺序取: **离线词典** (6748 条) > **日式罗马音** > **大模型校正** > **免费接口** > **英文音译规则**, 顺序可以在设置面板里上下调
...
- 记号 (`D/N/A` / `R&B` / `X-Y` / `M・I・D・I`) 按**字母名**逐字母读, 而且**一个字母一个注音** (`&` 读 アンド, 分隔符不发音); 缩写 (`you're` / `I'll` / `don't`) 拆成"词干 + 尾巴"; 不发音字母也认 (`knock` ノック, `climb` クライム, `subtle` サトル)
