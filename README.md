...
- 读音按顺序取：**离线词典**（6169 条）→ **罗马音** → **大模型校正** → **免费接口** → **英文音译规则**，这个顺序**可以自己调**（设置面板「读音来源顺序」的 ↑↓）：把*英文音译规则*提到在线层前面 = 一个请求都不发（纯离线）；把*大模型*提到词典前面 = 连词典命中的词也让模型判一遍。规则只当垫底：在线那层还没回来时先给一个**暂定的规则读音**（样式淡到 45%，`lt-pending`），拿到结果**就地改写**、不重建节点。罗马音层遇到**看着像英文词**的（`shake`→シャケ、`open`→オペン）会标成"没把握"，交给在线层仲裁，不会就此拍板
...
- 只标歌词原文行和播放栏标题，**含汉字的行也照标**；中文翻译层（默认歌词页 `li.line` 里的第二个块、RNP 的 `-translated`）和制作信息行（作词/作曲/编曲…）跳过。换歌时上一首的注音会撤掉、不会留在新歌的行里，新歌那一行照常注音（网易云换歌时会复用同一批歌词元素，这两条都是踩过坑的）
